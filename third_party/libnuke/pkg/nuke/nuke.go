...
func (n *Nuke) Run(ctx context.Context) error {
	...
	// entigo patch: count ItemStateNewDependency too. runScanner marks every item of a
	// type that *declares* DependsOn as ItemStateNewDependency - even when the depended-on
	// types have zero items left - so a scan where all remaining resources belong to
	// dependency-declaring types (the tail end of a partially-completed nuke: subnets,
	// route tables, NSGs, gateways) counted 0 here and falsely reported nothing to delete.
	// HandleWaitDependency already handles the "dependencies actually gone" case by
